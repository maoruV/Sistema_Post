...
- **Frontend**: Tailwind CSS via CDN, HTMX for partial updates, Alpine.js for client-side state. `app.css` has full design system with CSS vars, grain texture overlay, animations.
...
- **Reports/dashboard**: only count sales with `status='pagada'`. All date filters use `date__gte`/`date__lt` (sargable).
